...
\svgFigure{erd-update}{Verabredung}{350pt}
\pagebreak
...
\begin{figure}
	\centering
	\includegraphics[width = \columnwidth]{treff2.pdf}
	\captionof{figure}{Sämtliche Klassen und Packages des treff-Clients. Eine 
...
beinhaltet in erster Linie die Standorte von befreundeten Usern oder andere 
Mitglieder in Gruppen, aber auch Events werden hier angezeigt. Dabei beruht die 
Darstellung der Karte auf einer MapView, welche von der Open-Street-Map (OSM) 
zur Verfügung gestellt wird. Diese bietet auch bereits die Möglichkeit, 
unterschiedliche Markierungen auf der Karte anzubringen. Um die relevanten 
Daten wie Nutzerstandorte bereit zu stellen kommt wieder ein angepasstes 
...
Filter kann vom User über einen Android-eigenen Dialog angepasst werden.
...
%TODO Settings, die ganzen Adds, Login...
...
relevante Daten auf dem Gerät zu cachen und zu verhindern, dass alle 
Informationen stets vom Server geholt werden müssen. Sie wird von einer Android 
Library namens Room erstellt, welche zugleich eine Abstraktionsebene zu dieser 
Datenbank bereitstellt. Diese erlaubt es, fließend mit der Datenbank zu 
interagieren. Dazu werden sog. Data Access Objects (DAOs) verwendet. Diese 
wandeln Aufrufe von Java-Methoden in SQL-Queries um Daten in für Java 
sinnvollen Formaten aus der Datenbank zu lesen. Im Fall von treff. sollen z.B. 
neben diversen anderen vor allem Nutzer-, Gruppen-, und Event-Details angezeigt 
...
ihnen eines für jeden Datentyp, den es zu speichern gilt. Repositories werden 
direkt vn den ViewModels angessprochen und ihre Aufgabe ist es, Daten für diese 
...
\begin{figure}
	...
	Methoden wurden für eine bessere Lesbarkeit entfernt.}
	\label{fig:model}
\end{figure}
...
fehlerhaftem Userinput und InternalError bei internem Problem). Je nach 
...
entgegengenommen auf der Datenbank ausgeführt und die Ergebnisse zum Encodieren
...
\subsection{Abrufen von Aktualisierungen}
Zum Abrufen von Aktualisierungen gibt es mehrere Möglichkeiten.

\begin{itemize}
    \item Einzelne Anfrage an den Server, die alle aktuell für das eigene
        Benutzerkonto vorliegenden Änderungen abruft
    \item Anfrage an den Server, die die aktuelle Verbindung als
    ...
    \item Push-Notifications.
        Diese Möglichkeite der Übertragung ist auf mehreren
        ...
        weitere quelloffene Kandidaten.
        Die Umsetzbarkeit dieser Übertragungsmöglichkeit wird sich
        wahrscheinlich erst während der Implementierung zeigen.
\end{itemize}
