...
Die Login Activity wird bei jedem neuen Start der Anwendung geöffnet. Sie ist 
...
Ladebildschirms dar. Ist die Synchrinisation erfolgreich, startet sie die 
HomeActivity und beendet sich selbst. Tritt ein Authentifizierungsproblem auf, 
weil zB. aktuell gar kein, oder ein nicht gültiger Token vergeben ist, wird der 
Nutzer auf den eigentlichen Login Bildschirm weitergeleitet. Sowohl der Login, 
als auch der Registrierungsbildschirm werden durch Fragments realisiert. Per 
Knopfdruck kann jeweils das andere Fragment angezeigt werden. Hat der 
Nutzer seine Nutzerdaten vollständig eingegeben und den registrieren, bzw. 
login Knopf betätigt, werden die Daten durch das Fragment an den 
Requestencoder übergeben. Dieser gibt dann zurück, ob die Anfrage erfolgreich 
war. Dementsprechend wird der Nutzer zu der Home Activity weitergeleitet, oder 
ein für den Nutzer verständlicher Fehler wird durch das aktuelle Fragment 
angezeigt.
...
Die lokale SQLite-Datenbank hat die Aufgabe, alle für den konkreten Klient 
...
verfüg, ist dieser Zwischenschritt über eine Eigene Klasse notwendig. Denn so 
...
anzuzeigen, währen die Übertragung an den Server nicht erwünscht ist. Zudem 
...
Klient behandelt werden muss. Diese Aufgabe übernimmt der RequestEncoder. 
...
Klienten.
Da die Anfragen eines Benutzer sequentiell abgearbeitet werden sollen, ruft
...
AccountUpdateListener-Implementation PersistentConnection zuständig.
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width = \columnwidth / 100 * 90]{treff3.pdf}
