...
public abstract class PollOptionDependentTest extends PollDependentTest{
    ...
    protected final long timeStart = 0,timeEnd = Long.MAX_VALUE;
    ...
    protected void checkPollOptionUpdateForUser(User user,
        ...
        Assert.assertEquals(latitude,updatePollOptionDesc
                .getJsonNumber("latitude").doubleValue(),0);
        Assert.assertEquals(longitude,updatePollOptionDesc
                .getJsonNumber("longitude").doubleValue(),0);
        Assert.assertEquals(timeStart,updatePollOptionDesc
                .getJsonNumber("time-start").longValue());
        Assert.assertEquals(timeEnd,updatePollOptionDesc
                .getJsonNumber("time-end").doubleValue());
    }
}
